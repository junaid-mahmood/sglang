...
class TestTrailSorting(unittest.TestCase):
    ...
    def test_lrpsprpt_unpreemptible_requests_first(self):
        # r1: generated 90/100 tokens (90% > 80% threshold) → un-preemptible
        r1 = make_req(
            "a",
            output_ids=[0] * 90,
            trail_state=make_trail_state(initial=100, remaining=10),
        )
        # r2: generated 10/100 tokens → preemptible, remaining=90
        r2 = make_req(
            "b",
            output_ids=[0] * 10,
            trail_state=make_trail_state(initial=100, remaining=90),
        )
        # r3: generated 50/100 tokens → preemptible, remaining=50
        r3 = make_req(
            "c",
            output_ids=[0] * 50,
    ...
    def test_lrpsprpt_threshold_boundary(self):
        ...
        queue = [r2, r1]
        SchedulePolicy._sort_by_trail_lrpsprpt(queue, preemption_threshold=0.8)
        # At boundary (80/100 = 0.8, 0.8 > 0.8 is False) → preemptible
        # Actually 80 > 80 is False, so at_boundary is preemptible too
        # Need > threshold, not >=
        # Both preemptible, sort by remaining: at_boundary(20) < below(21)
        self.assertEqual([r.rid for r in queue], ["at_boundary", "below"])

    def test_lrpsprpt_past_threshold_is_unpreemptible(self):
        # 81 > 0.8*100=80 → un-preemptible
        r1 = make_req(
            "past",
            output_ids=[0] * 81,
...
class TestTrailCompare(unittest.TestCase):
    ...
    def test_no_preempt_when_running_has_shorter_remaining(self):
        ...
        self.assertLessEqual(
            result, 0, "Should not preempt: running has shorter remaining"
        )
    ...
    def test_no_preempt_when_running_is_unpreemptible(self):
        ...
        result = SchedulePolicy.trail_compare(
            waiting, running, preemption_threshold=0.8
        )
        self.assertLessEqual(
            result, 0, "Should not preempt: running past 80% threshold"
        )
    ...
    def test_no_preempt_when_waiting_has_no_prediction_short_running(self):
        ...
        self.assertLessEqual(
            result, 0, "Should not preempt: running remaining not clearly long"
        )
    ...
    def test_preempt_when_waiting_has_no_prediction_long_running(self):
        ...
        self.assertGreater(
            result, 0, "Should preempt: running has clearly long remaining"
        )
    ...
    def test_no_preempt_when_running_has_no_prediction(self):
        ...
        result = SchedulePolicy.trail_compare(waiting, running)
        self.assertLessEqual(
            result, 0, "Should not preempt: running has no prediction yet"
        )
    ...
    def test_custom_threshold(self):
        ...
        # With threshold=0.5, 55 > 50 → un-preemptible
        result = SchedulePolicy.trail_compare(
            waiting, running, preemption_threshold=0.5
        )
        self.assertLessEqual(result, 0, "Should not preempt: past 50% threshold")
        # With threshold=0.8, 55 < 80 → preemptible
        result = SchedulePolicy.trail_compare(
            waiting, running, preemption_threshold=0.8
        )
        self.assertGreater(
            result, 0, "Should preempt: below 80% threshold, waiting shorter"
        )
